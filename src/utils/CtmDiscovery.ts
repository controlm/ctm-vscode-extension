...
export async function getCtmInfrastructure(context: vscode.ExtensionContext) {
	...
	if (bValidConnection) {
		...
		servers.forEach((e: any, i: any) => {
			// Get CTM server [datacenter] details
		...
		jsonStr = '{"Selected": "' + strConfiguredEnvironmentName + '",' + jsonEnvironments + ',"discovery": "' + ctmDiscoverDate.toString() + '","age": "","datacenters": [' + jsonStrE + '],' + jsonConnectionProfilesCentralized + ',' + jsonWorkloadPolicies + ',' + jsonSecrets + ',"elapsed":"' + elapsed + '","status":"success"}';
 ...
 * @param sProcNum Output channel message prefix
 */
export async function getConnectionProfileTypes(envCmd: string, sProcNum: string) {
	// Command will return with error, this is on purpose
