...
export async function activate(context: vscode.ExtensionContext) {
    ...
    const ctmInfrastructureFolder: string = vscode.workspace.workspaceFolders[0].uri.fsPath; // rootPath;
    ...
    if (jsonDataStatus === undefined) {
        ...
        if (jsonDataStatus.startsWith("success")) {
            ctmDataStatus = true;
            if (debugEnabled) {
                console.log(' - JSON Cache Datafile: ' + ctmInfrastructureFile.toString() + '"');
            }
    ...
    if (ctmDataStatus) { // calulate age of report
    ...
    // CTM Infrastructure
    let ctmInfrastructureRefreshEntry = vscode.commands.registerCommand('ctm.infrastructure.refreshEntry', async () => {
        vscode.window.showInformationMessage("Discovering Control-M infrastructure by means of the Automation API will take a few minutes and block the UI. Do you want to continue?", "Discover Control-M", "Cancel", "Help").then(async answer => {
    ...
    // CTM Infrastructure
    let ctmInfrastructureDemoDataLoad = vscode.commands.registerCommand('ctm.infrastructure.demo.data.load', async () => {
        vscode.window.showInformationMessage("Load Control-M Demo Data. Control-M Infrastructure interaction limited. Do you want to continue?", "Load Control-M Demo Data", "Cancel").then(async answer => {
    ...
    // CTM Self-Service
    let ctmSelfService = vscode.commands.registerCommand('ctm.infrastructure.self.service', async () => { // get url from extension globbal config
    ...
    let ctmApplicationIntegrator = vscode.commands.registerCommand('ctm.infrastructure.application.integrator', async () => { // get url from extension globbal config
    ...
    let ctmSharedConnectionProfile = vscode.commands.registerCommand('ctm.infrastructure.shared.connection.profile', async () => { // get url from extension globbal config
    ...
    let ctmConfiguration = vscode.commands.registerCommand('ctm.infrastructure.configuration', async () => { // get url from extension globbal config
    ...
    // CTTM Calendars
    let ctmCalendars = vscode.commands.registerCommand('ctm.infrastructure.calendars', async () => { // get url from extension globbal config
    ...
    // CTM Session Login
    let ctmSessionLogin = vscode.commands.registerCommand('ctm.activity.bar.session.login', async () => {
        vscode.window.showInformationMessage('CTM Session Login');
    });
    context.subscriptions.push(ctmSessionLogin);

    // CTM Documentation onPrem
    let ctmDocsOnPrem = vscode.commands.registerCommand('ctm.activity.bar.documentation.on.prem', async () => {
    ...
    let ctmDocsSaaS = vscode.commands.registerCommand('ctm.activity.bar.documentation.saas', async () => {
    ...
    // CTM Swagger
    context.subscriptions.push(vscode.commands.registerCommand('ctmSwagger.start', () => {
        ctmSwaggerPanel.createOrShow(context.extensionUri);
    }));

    context.subscriptions.push(vscode.commands.registerCommand('ctmSwagger.doRefactor', () => {
        if (ctmSwaggerPanel.currentPanel) {
            ctmSwaggerPanel.currentPanel.doRefactor();
        }
    }));

    if (vscode.window.registerWebviewPanelSerializer) { // Make sure we register a serializer in activation event
 ...
 */
async function getSelectedFileUris(): Promise<vscode.Uri[]> { // get what's currently on the clipboard
...
function getWebviewOptions(extensionUri: vscode.Uri): vscode.WebviewOptions {
    return { // Enable javascript in the webview
        enableScripts: true,
